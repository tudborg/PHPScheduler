...
class RedisBackendTest extends \PHPUnit_Framework_TestCase
{
    public function testRedisBackend()
    {
        ...
        //schedule tasks
        $this->assertTrue($backend->schedule($original_t2, microtime(true)-1)); //schedule t2 first, in the past
        $this->assertTrue($backend->schedule($original_t3, microtime(true))); //and t3 now
        $this->assertTrue($backend->schedule($original_t1, microtime(true)-2)); //and t1 before any of the above
